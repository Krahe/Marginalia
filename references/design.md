...
1. **~~Auto-injection at spawn~~ — SHIPPED as practice (default-on).** The orchestrator injects the communion read + location notes by default, and a standing memory rule makes that the default rather than a thing remembered. *Remaining:* a true harness hook (or MCP) so injection is *structural* — enforced regardless of whether the orchestrator remembers — instead of practice routed through one mind's discipline.
2. **Project resolution hardening** — `--project` defaults to cwd basename; an agent run from the wrong dir mislabels notes silently (caught by a witness agent on the first run). Options: require explicit `--project`, warn loudly on default (current), or resolve to a git-root marker.
3. **Per-project stores** (`<repo>/.marginalia/`) vs the single global store — cleaner scoping, more plumbing. Global-with-keys is fine for now.
