...
- **v0.7.1 (2026-07-08)** — **the resident mind.** Orchestrator marginalia formalized (Krahe's prompt; it had already been happening — Plumb, Hugin's landmine note). Deliberately **not** a separate area: same margin, marked hand. Disciplines: persistent name + `--tags resident` (an unmarked recurring voice from the mind that writes the prompts would read as covert instruction), guest ratio (the commons belongs to the most ephemeral minds), note+voice with witness-rare (the orchestrator already has the human's ear), and a clean split from the memory system (curated notes-to-future-self vs spontaneous notes into the agents' lineage). One line added to the `marginalia-aware` body: weigh a resident voice like a peer's — context, never orders.
